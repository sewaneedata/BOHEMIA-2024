...
ac_effi <- kenya_efficacy_total %>%
  group_by(visit, sleep_under_net_last_night) %>%
  tally() %>%
  drop_na(sleep_under_net_last_night)
ac_effi<-ac_effi %>%
  dplyr::filter(sleep_under_net_last_night == 'yes')
ts_data_effi <- timeSeries::timeSeries(ac_effi$n, start = 1, units = "V")
ts_data_effi <- ts(ts_data_effi, start = 1)
autocorr_result_eff <- acf(ts_data, plot = TRUE)
autocorr_result_eff
