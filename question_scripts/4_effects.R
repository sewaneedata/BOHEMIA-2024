...
weather_p<- gsheet::gsheet2tbl('https://docs.google.com/spreadsheets/d/1CYuQ6_QZGaLft1zuRRdy-WxARICRzbGL9P7MSxd9Klk/edit?usp=sharing')
...
#selecting only the months from the survey
weather_k_p<-weather_k_p %>% 
  filter(month %in% c('October', 'November', 'December', 'January'))


#getting average temp for each month
weather_k_t_p<-weather_k_p %>% 
  group_by(month) %>% 
  summarize(avg_temp=mean(TAVG))
#getting sum of prcp for each month
weather_k_p_p<-weather_k_p %>% 
  group_by(month) %>% 
  summarize(sum_prcp=sum(PRCP, na.rm=TRUE))

#adding the two tables
weather_t_p_p<-weather_k_p_p %>% 
  left_join(weather_k_t, by='month')


#arranging months in order
weather_t_p_p$month <- factor(weather_t_p_p$month, 
                            levels = c("October", "November", "December", "January"))


#creating graph for temp
ggplot(weather_t_p_p, aes(x=month, y = avg_temp, group=1)) +
  geom_line()

#creating graph for prcp

ggplot(weather_t_p_p, aes(x=month, y = sum_prcp, group=1)) +
  geom_line()
...
#rescaling prcp to temp
weather_t_p_p$avg_temp_rescaled <- rescale(weather_t_p_p$avg_temp, to = range(weather_t_p_p$sum_prcp))


#creating the plot
weather_graph_twodata<-ggplot(weather_t_p_p, aes(x = month)) +
        geom_line(aes(y = avg_temp_rescaled, color='Temperature'), group=1) +
        geom_line(aes(y = sum_prcp, color='Precipitation'), group=1) +
        geom_col(
          data = kenya_safety_eff_long, 
          aes(y = number / max(number) * max(weather_t_p_p$sum_prcp),  # Rescale and normalize safety data
              fill = percent_y_n), 
          position = "dodge", 
          alpha=.5
        ) +
        scale_y_continuous(
          ...
          limits = c(0, max(weather_t_p_p$avg_temp_rescaled) * 1)
        ) +
        scale_color_manual(values = c("Temperature" = "#9666B2", 
                                      "Precipitation" = "#000000"))+
        scale_fill_manual(values = c("percent_yes_eff" = "#4B0082", "percent_yes_safety" = "#9666B2"),
...
#getting sum of prcp for each month
weather_e_p_p<-weather_e_p %>% 
  group_by(month) %>% 
  summarize(sum_prcp=sum(PRCP, na.rm=TRUE))

#adding the two tables
weather_e_t_p_p<-weather_e_p_p %>% 
  left_join(weather_e_t_p, by='month')

#arranging months in order
weather_e_t_p_p$month <- factor(weather_e_t_p_p$month, 
                              levels = c("October", "November", "December", "January", "February", "March", "April"))

weather_e_t_p_p$avg_temp_rescaled <- rescale(weather_e_t_p_p$avg_temp, to = range(weather_e_t_p_p$sum_prcp))
...
# customizing ggplot
p<-ggplot(weather_e_t_p_p, aes(x = month)) +
  ...
  geom_col(
    ...
    aes(y = number / max(number) * max(weather_e_t_p_p$sum_prcp)), 
  ...
  scale_y_continuous(
    ...
    limits = c(0, max(weather_e_t_p_p$avg_temp_rescaled) * 1)
  ) +
  scale_color_manual(values = c("Temperature" = "#9666B2", 
                                "Precipitation" = "#000000")) + 
